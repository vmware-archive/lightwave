...
{
    public class LocalFileStorage : IPersistentStorage<SimpleCache<AuthTokenDto>>
    {
        ...
        public void Save(SimpleCache<AuthTokenDto> store)
        {
            using (var ms = new MemoryStream())
            {
                var xmlSerializer = new XmlSerializer(typeof(SimpleCache<AuthTokenDto>));
                xmlSerializer.Serialize(ms, store);
                File.WriteAllBytes(_fileName, ms.ToArray());
            }
        }

        public SimpleCache<AuthTokenDto> Load()
        {
            ...
            try
            {
                using (var ms = new MemoryStream())
                {
                    ...
                    var xmlSerializer = new XmlSerializer(typeof(SimpleCache<AuthTokenDto>));
                    return xmlSerializer.Deserialize(ms) as SimpleCache<AuthTokenDto>;
                }
            }
            catch (Exception)
            {

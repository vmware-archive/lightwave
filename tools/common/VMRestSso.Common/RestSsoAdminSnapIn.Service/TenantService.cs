...
namespace Vmware.Tools.RestSsoAdminSnapIn.Service.Tenant
{
    public class TenantService
    {
        ...
        public TenantService(IWebRequestManager webRequestManager)
        {
        ...
        public TenantDto Get(ServerDto serverDto, string tenant, Token token)
        {
            ...
            var url = string.Format(ServiceConfigManager.GetTenantPostEndPoint, serverDto.Protocol, serverDto.ServerName, serverDto.Port, tenant);
        ...
        {
            tenant = Uri.EscapeDataString(tenant);
            var url = string.Format(ServiceConfigManager.TenantEndPoint, serverDto.Protocol, serverDto.ServerName, serverDto.Port, tenant);
            ServicePointManager.ServerCertificateValidationCallback = delegate { return true; };
            var requestConfig = new RequestSettings
                                    {

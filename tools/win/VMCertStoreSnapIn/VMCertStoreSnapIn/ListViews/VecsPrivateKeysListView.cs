...
{
    public class VecsPrivateKeysListView : VecsStoreEntriesListView
    {
        const int ACTION_SHOW_CERTIFICATE = 1;
        const int ACTION_SHOW_PRIVATE_KEY = 2;
        ...
        {
            if (!MMCDlgHelper.ShowConfirm(CommonConstants.GetSelectedDeleteMsg("private entities", this.SelectedNodes.Count)))
                return;
        ...
        public override void Refresh()
        {
            ...
            MMCActionHelper.CheckedExec(delegate()
            {
                using (var session = new VecsStoreSession(dto.VecsClient, storeName, storePass))
                {
                    var lst = new List<Vecs.CertDTO>();
                    lst.AddRange(session.GetPrivateKeys());
                    lst.AddRange(session.GetEncryptedPrivateKeys());

                    foreach (var certDTO in lst)
                    {
                        var resultNode = new ResultNode { DisplayName = certDTO.Alias };
                        resultNode.ImageIndex = (int)VMCertStoreImageIndex.PrivateEntities;
        ...
        protected override void OnSelectionChanged(SyncStatus status)
        {
            ...
            else
            {
                ...
                /*this.SelectionData.ActionsPaneItems.Add(new Microsoft.ManagementConsole.Action("Show Private key",
                                                       "Shows private key", -1, ACTION_SHOW_PRIVATE_KEY));*/
            }
        }
        ...
        {
            var certDTO = this.SelectedNodes[0].Tag as Vecs.CertDTO;
            if (certDTO.PrivateKey != null)
            {
                var frm = new InfoForm(certDTO.PrivateKey);

...
if [ $# -lt 2 ]; then
    echo "Usage: build-lightwave-client-container.sh <path: docker src root> <path: docker image>"
    exit 1
fi
...
DOCKER_IMAGE_TAG="vmware/lightwave-client"
...
# build vmware/lightwave-client docker image

    ...
    )
{
    ...
    dwError = VmDirAttributeMetaDataToHashMap(
            pAttrAttrMetaData,
            &pMetaDataMap);
    assert_int_equal(dwError, 0);
    assert_non_null(pMetaDataMap);

    while(LwRtlHashMapIterate(pMetaDataMap, &iter, &pair))
    {
        pszAttrType = (PSTR) pair.pKey;
        pMetaData = (PVMDIR_ATTRIBUTE_METADATA) pair.pValue;

        if (dwCnt == 0)
        {
            assert_string_equal(pszAttrType, "cn");
            assert_int_equal(pMetaData->localUsn, 100);
            assert_int_equal(pMetaData->version, 23);
            assert_string_equal(pMetaData->pszOrigInvoId, "7ef77c0f-cff1-4239-b293-39a2b302d5bd");
            assert_string_equal(pMetaData->pszOrigTime, "20180702222545.584");
            assert_int_equal(pMetaData->origUsn, 100);
        }
        else if (dwCnt == 1)
        {
            assert_string_equal(pszAttrType, "sn");
            assert_int_equal(pMetaData->localUsn, 110);
            assert_int_equal(pMetaData->version, 25);
            assert_string_equal(pMetaData->pszOrigInvoId, "7ef77c0f-cff1-4239-b293-39a2b302d5bd");
            assert_string_equal(pMetaData->pszOrigTime, "20180702222545.584");
            assert_int_equal(pMetaData->origUsn, 105);
        }
        else
        {
            assert_false(TRUE);
    ...
    )
{
    ...
    //Invalid input
    VMDIR_SAFE_FREE_MEMORY(pAttrAttrMetaData->vals[1].lberbv.bv_val);
    dwError = VmDirAllocateStringA("cn:100:23", &pAttrAttrMetaData->vals[1].lberbv.bv_val);
    assert_int_equal(dwError, 0);
    ...
    dwError = VmDirAllocateStringA("cn:100:", &pAttrAttrMetaData->vals[0].lberbv.bv_val);
    assert_int_equal(dwError, 0);

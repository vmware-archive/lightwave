...
{
    BOOLEAN bSensitiveAttr = TRUE;

    if (!pszAttrName)
    {
        bSensitiveAttr = FALSE;
    }
    else
    {
        if (VmDirStringCompareA(pszAttrName, ATTR_USER_PASSWORD, FALSE)     != 0 &&
            VmDirStringCompareA(pszAttrName, ATTR_SRP_SECRET, FALSE)        != 0 &&
            VmDirStringCompareA(pszAttrName, ATTR_KRB_PRINCIPAL_KEY, FALSE) != 0)
        {
            bSensitiveAttr = FALSE;
        }
    }

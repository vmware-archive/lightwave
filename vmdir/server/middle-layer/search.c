...
/*
 * If the pszBindDN is a member of pszGroupDN entry, *pbIsMemberOf = TURE;
 * P.S. This function does not consider nested group membership scenario.  It only
    ...
    )
{
    ...
    if ( !pszBindDN || !pszGroupDN || !pbIsMemberOf )
    {
        dwError = VMDIR_ERROR_INVALID_PARAMETER;
        BAIL_ON_VMDIR_ERROR(dwError);
    }
    ...
    )
{
    ...
    VmDirLog( LDAP_DEBUG_FILTER, "Filter choice: %ld\n", f->choice, 0, 0 );
        ...
        case LDAP_FILTER_AND:
        {
            ...
            else
            {
                ...
                for (nextFilter = f->filtComp.complex;
                     nextFilter != NULL && bStopFilter == FALSE;
                     nextFilter = nextFilter->next)
                {
                    ...
                    if (nextFilter->candidates != NULL                                      &&
                        ...
                        nextFilter->candidates->size < MAX_GOOD_CANDIDATE_SIZE              &&
    ...
    )
{
    ...
    if (cl && cl->size > 0)
    {
             ...
             i++)
        {
            ...
            VMDIR_LOG_VERBOSE( LDAP_DEBUG_FILTER, "ProcessCandidateList EID(%u)", cl->eIds[i]);

    ...
    for (i = 0; i < METRICS_LDAP_OP_COUNT; i++)
    {
        for (j = 0; j < METRICS_LDAP_OP_TYPE_COUNT; j++)
        {
            for (k = 0; k < METRICS_LDAP_ERROR_COUNT; k++)
            {
                ...
                dwError = VmMetricsHistogramNew(
                        pmContext,
                        "vmdir_ldap_middlelayer_preplugins",
                        labels, 2,
                ...
                dwError = VmMetricsHistogramNew(
                        pmContext,
                        "vmdir_ldap_middlelayer_writequeue",
                        labels, 2,
                        "Histogram for LDAP request durations in the writequeue",
                        buckets, 8,
                        &gpLdapMetrics[i][j][k][METRICS_LAYER_WRITE_QUEUE]);
                BAIL_ON_VMDIR_ERROR(dwError);
                ...
                dwError = VmMetricsHistogramNew(
                        pmContext,
                        "vmdir_ldap_middlelayer_backend_txnbegin",
                        labels, 2,
                ...
                dwError = VmMetricsHistogramNew(
                        pmContext,
                        "vmdir_ldap_middlelayer_backend_txncommit",
                        labels, 2,
                ...
                dwError = VmMetricsHistogramNew(
                        pmContext,
                        "vmdir_ldap_middlelayer_postplugins",
                        labels, 2,

...
public final class LdapConnectionPool {
    ...
    private final static String SYSTEM_TENANT = "SystemTenant-" + RandomStringUtils.randomAscii(5);
	...
	GenericKeyedObjectPool<PooledLdapConnectionIdentity, ILdapConnectionEx> systemTenantPool = new GenericKeyedObjectPool<>(
		new PooledLdapConnectionFactory(), getGenericKeyedObjectPoolConfig(SYSTEM_TENANT));
	this.poolMap = new ConcurrentHashMap<>();
	this.poolMap.put(SYSTEM_TENANT, systemTenantPool);
    }
	...
	String tenantName = identity.getTenantName() == null ? SYSTEM_TENANT : identity.getTenantName().toLowerCase();
	...
	if (pooledConnection == null || pooledConnection.getIdentity() == null
		|| pooledConnection.getConnection() == null)
	    return;

	String tenantName = pooledConnection.getIdentity().getTenantName() == null ? SYSTEM_TENANT : pooledConnection
		.getIdentity().getTenantName().toLowerCase();
	...
	GenericKeyedObjectPool<PooledLdapConnectionIdentity, ILdapConnectionEx> currentPool = poolMap.get(tenantName.toLowerCase());

	if (currentPool == null) {
	    newPool = new GenericKeyedObjectPool<>(new PooledLdapConnectionFactory(), getGenericKeyedObjectPoolConfig(tenantName));
	    currentPool = poolMap.putIfAbsent(tenantName.toLowerCase(), newPool);
	    if (currentPool != null) {
		newPool.close();
	    }
	    logger.info(currentPool != null ? "Pool created for tenant " + tenantName : "Pool already existed for tenant " + tenantName);
	}
    }

    public void cleanPool(String tenantName) {
	GenericKeyedObjectPool<PooledLdapConnectionIdentity, ILdapConnectionEx> pool = poolMap.remove(tenantName.toLowerCase());
	if (pool != null) {
	    pool.close();

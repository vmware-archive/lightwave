...
/**
 * @author root Utility class for client library unit test. Signing keys: a
 *         keystore resource is build in the project. We could replace it with a
 *         temp ks in tmp folder to overide the default ks in the project.
 * 
 */
public class TestUtils {
    private static Logger logger = LoggerFactory.getLogger(TestUtils.class);

    // Key information for singing. These will be used for creating simulated
    // singedSAMLObject
    ...
    /**
     * This algorithm uri assciate to the algorithm used in "/sts-store.jks". I am
     * not sure how to get from the resource grammatically. Hence hard code as
     * constant here.
     */
     ...
     * 
     * What it does: init keystore and key/cert for signing and verification test.
     * 
     */
    public static void bootstrap() throws Exception {
        ...
        if (signingKey == null || ks == null) {
            ...
            ks.load(TestUtils.class.getResourceAsStream("/sts-store.jks"), stsKeystorePassword);
     ...
     */
    public static String getParameterFromQueryMap(Map<String, String> queryMap, String parameter)
    ...
    static public MetadataSettings CreateMetadataSettings() {
        ...
        IDPConfiguration idpConfiguration = new IDPConfiguration(TestConfig.tenantName, TestConfig.idpEntityID,
                TestUtils.getCertificate(), idFormats, ssoServices, idpSloServices);
        ...
        AssertionConsumerService asco = new AssertionConsumerService(TestConfig.ACS0_endpoint, true,
                TestConfig.ACS0_binding, 0);
        ...
        SPConfiguration spConfiguration = new SPConfiguration(TestConfig.tenantName, TestConfig.spEntityID,
                TestConfig.authnSigned, // athu
                signingKey, // signing key
                null, TestConfig.SpSign_algorithm_URI, // signingAlgorithm,
                spIDFormats, ascoServices, spSloServices);
    ...
    /**
     * Build a saml sso response with given token This function create an instance
     * of SamlUtils and populate call to SamlUtils::createSamlLoginResponse with
     * test parameters.
     * 
     * @param inResponseTo
     *            the request id this response is reponsing to
     * @param token
     *            assertion dom
     * @return Response object created.
     */
    static public Response createSamlAuthnResponse(String inResponseTo, Document token)
            throws NoSuchAlgorithmException, UnmarshallingException {
        ...
        // create SAML request
        return samlUtils.createSamlLoginResponse(inResponseTo, destination, SamlNames.SUCCESS, null, null, // message
                token);
    }
     ...
     */
    public static LogoutRequest createSamlLogoutRequestFromIDP(String id, String sessionIndex)
            throws NoSuchAlgorithmException {
        // get parameters
        ...
        // create SAML request
        return samlUtils.createSamlLogoutRequest(id, destination, SamlNames.PERSISTENT, TestConstants.USER,
                sessionIndex);
    }
     ...
     *
     * @param inResponseTo
     *            logout request id it responding to.
     * @param sessionIndex
     * @return
     * @throws NoSuchAlgorithmException
     */
    public static LogoutResponse createSamlLogoutResponseFromIDP(String inResponseTo, String sessionIndex)
            throws NoSuchAlgorithmException {
        // get parameters
        String issuerUrl = TestConfig.idpEntityID;
        String destination = TestConfig.SpSloService_loc;

        return SamlUtils.createSamlLogoutResponse(issuerUrl, inResponseTo, // inResponseTo
                destination, // destination
                StatusCode.SUCCESS, StatusCode.SUCCESS, null);
     ...
     *
     */
    public static HttpServletRequest buildMockHttpServletRequestObject(SignableSAMLObject samlObject,
            boolean doCompress, String relayStateParameter, String sigAlg, String signature, StringBuffer sbRequestUrl)
        ...
        if (samlObject instanceof LogoutResponse || samlObject instanceof Response) {
            expect(request.getParameter(SamlUtils.SAML_RESPONSE_PARAMETER))
                    .andReturn(SamlUtils.encodeSAMLObject(samlObject, doCompress)).anyTimes();
            expect(request.getParameter(SamlUtils.SAML_REQUEST_PARAMETER)).andReturn(null).anyTimes();
        } else if (samlObject instanceof LogoutRequest) { //
            expect(request.getParameter(SamlUtils.SAML_REQUEST_PARAMETER))
                    .andReturn(SamlUtils.encodeSAMLObject(samlObject, doCompress)).anyTimes();
            expect(request.getParameter(SamlUtils.SAML_RESPONSE_PARAMETER)).andReturn(null).anyTimes();
        } else {
            // any other obj, throw.
            throw new IllegalArgumentException();
        }

        expect(request.getParameter(SamlUtils.RELAY_STATE_PARAMETER)).andReturn(relayStateParameter).anyTimes();
        expect(request.getParameter(SamlUtils.SIGNATURE_ALGORITHM_PARAMETER)).andReturn(sigAlg).anyTimes();
        expect(request.getParameter(SamlUtils.SIGNATURE_PARAMETER)).andReturn(signature).anyTimes();
        expect(request.getRequestURL()).andReturn(sbRequestUrl).anyTimes();

        String queryString = null;
        if (samlObject instanceof LogoutResponse || samlObject instanceof Response) {
            queryString = SamlUtils.SAML_RESPONSE_PARAMETER + "=" + SamlUtils.encodeSAMLObject(samlObject, doCompress);
        } else if (samlObject instanceof LogoutRequest) {
            queryString = SamlUtils.SAML_REQUEST_PARAMETER + "=" + SamlUtils.encodeSAMLObject(samlObject, doCompress);
        } else {
            // any other obj, throw.
            throw new IllegalArgumentException();
        }
        ...
        if (relayStateParameter != null) {
            queryString = queryString + "&" + SamlUtils.RELAY_STATE_PARAMETER + "=" + relayStateParameter;
        }
        if (sigAlg != null) {
            queryString = queryString + "&" + SamlUtils.SIGNATURE_ALGORITHM_PARAMETER + "=" + sigAlg;
        }
        if (signature != null) {
            queryString = queryString + "&" + SamlUtils.SIGNATURE_PARAMETER + "=" + signature;
        }
    ...
    /**
     * Return a mocked HttpServletResponse for successfully validating SSO or SLO
     * response received. No message is expected to send out via this response.
     */
    public static HttpServletResponse buildMockResponseObjectForResponse(final StringWriter sw) throws IOException {
     ...
     * 
     * @param tokenResource
     *            file name of the token in the resource folder.
     ...
     */
    public static Document readSignedToken() throws ParserConfigurationException, SAXException, IOException {
        logger.info("Creating assertion token from signed-assertion.xml. ");
        InputStream is = TestUtils.class.getResourceAsStream("/signed-assertion.xml");
        return SharedUtils.createDOM(is);
     ...
     * @throws UnsupportedEncodingException
     */
    public static String signMessage(String messageToSign)

...
public class IdpSsoController {
    ...
    public ModelAndView sso(Locale locale, @PathVariable(value = "tenant") String tenant, HttpServletRequest request,
        ...
        return sso(locale, tenant, request, response, factory);
    }

    ModelAndView sso(Locale locale, String tenant, HttpServletRequest request, HttpServletResponse response,
            IdmAccessorFactory factory) throws IOException {
        logger.info("Welcome to IDP-initiated AuthnRequest handler! " + "The client locale is " + locale.toString()
                + ", tenant is " + tenant);
        ...
        try {
            ...
            // create SAML request
            AuthnRequest authnRequest = service.createSamlAuthnRequest(null, // generate new id
                    destination, providerEntityID, nameIDFormat, index1, index2, b1, b2);
            ...
            stringBuilder.append(service.generateRedirectUrlQueryStringParameters(samlRequestParameter, null,
                    request.getParameter(Shared.RELAY_STATE_PARAMETER), null, null));
    ...
    /**
     * Default tenant is not enabled for IDP-initiated requests User MUST specify
     * relying party URL as part of request, then that means that user MUST know
     * tenant as well
     */

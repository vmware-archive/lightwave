 ...
 */
class AcquireTokenByTokenParserFactory extends RequestParserAbstractFactory<Element> {

    public AcquireTokenByTokenParserFactory(SecurityTokenServiceConfig config) {
        super(config);
    }

    @Override
    public RequestParametersValidator createRequestParametersValidator() {
        return new TokenParametersValidator(stsConfig);
    }
    ...
    public RequestBuilder createRequestBuilder(Credential clientCredential, TokenSpec tokenSpec) {
        if (!(clientCredential instanceof TokenCredential))
            throw new IllegalArgumentException("Credential type not valid for TokenCredentialFactory: "
        ...
        return new AcquireTokenByTokenRequestBuilder(cred.getToken(), tokenSpec, hokConfirmation, jaxbContext,
    ...
    public WsSecuritySignature createWsSecuritySignature(Credential clientCredential, TokenSpec tokenSpec) {
        ...
        TokenCredential tokenCredential = (TokenCredential) clientCredential;
        if (!tokenCredential.isExternal()) {
            return WsSecuritySignatureFactory.createWsSecuritySignatureAssertion(stsConfig.getHolderOfKeyConfig(),
                    tokenCredential.getToken().getId());
        } else {
            WsSecuritySignature wsSecuritySignature = null;
            if (tokenCredential.getToken().getConfirmationType() == ConfirmationType.HOLDER_OF_KEY) {
                wsSecuritySignature = WsSecuritySignatureFactory.createWsSecuritySignatureAssertion(
                        stsConfig.getHolderOfKeyConfig(), tokenCredential.getToken().getId());
            } else {
                wsSecuritySignature = getWsDefaultSecuritySignature(tokenSpec);
            }

            return wsSecuritySignature;
        }
    }
}

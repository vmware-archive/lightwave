...
IdmUserCreateTestByHOKToken()
{
    ...
    e = IdmUserCreate(pHOKTokenClient, tenant, pPrincipal, &pCreated, &pServerError);
    BAIL_ON_ERROR(e);
...
IdmUserGetTestByHOKToken()
{
    ...
    e = IdmUserGet(pHOKTokenClient, tenant, name, domain, &pUserReturn, &pServerError);
    BAIL_ON_ERROR(e);
...
PCSTRING
IdmUserDeleteTestByHOKToken()
{
    SSOERROR e = SSOERROR_NONE;
    REST_SERVER_ERROR* pServerError = NULL;

    e = IdmUserDelete(pHOKTokenClient, tenant, name, domain, &pServerError);
    BAIL_ON_ERROR(e);

    ...
    PCSTRING hokPrivateKeyFile)
{
    ...
    if (file != NULL)
    {
        ...
        while (fgets(line, sizeof(line), file) != NULL)
        {
            ...
            if (RestStringStartsWith(line, "server_ip"))
            {
            ...
            if (RestStringStartsWith(line, "server_port"))
            {
                serverPort = strtol(cfline, (PSTRING*) NULL, 10);
            }

            if (RestStringStartsWith(line, "bearer_token"))
            {
                e = SSOStringAllocate(cfline, &bearerAccessToken);
                BAIL_ON_ERROR(e)
            }

            if (RestStringStartsWith(line, "hok_token"))
            {

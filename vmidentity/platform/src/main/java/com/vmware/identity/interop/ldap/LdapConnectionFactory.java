...
{
    private static LdapConnectionFactory _instance = null;

    public static synchronized LdapConnectionFactory getInstance()
    {
        if (_instance == null)
        {
            _instance = new LdapConnectionFactory();
        }
     ...
     */
    public ILdapConnection getLdapConnection( String host, int port, List<LdapSetting> connOptions)
    {
     ...
     * @return
     */
    public ILdapConnection getLdapConnection(URI uri, List<LdapSetting> connOptions)
    {
     ...
     * @param uri
     * @param connOptions
     * @param useOpenLdap set to 'true' for using OpenLdap library (required for Windows platform only).
     *This is needed for SRP binding for Windows platform. For linux platform, OpenLdap is used always.
     * @return
     */
    public ILdapConnection getLdapConnection(URI uri,
            List<LdapSetting> connOptions, boolean useOpenLdap) {
        return new LdapConnection(uri, connOptions, useOpenLdap);
    }

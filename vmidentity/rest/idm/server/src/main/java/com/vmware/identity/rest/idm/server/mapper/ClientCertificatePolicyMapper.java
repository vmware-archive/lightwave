...
public class ClientCertificatePolicyMapper {
    ...
    public static ClientCertificatePolicyDTO getClientCertificatePolicyDTO(ClientCertPolicy clientCertPolicy) {
        ...
        if (clientCertPolicy.getTrustedCAs() != null) {
            trustedCertificates = CertificateMapper.getCertificateDTOs(Arrays.asList(clientCertPolicy.getTrustedCAs()));
        }
        ...
        return ClientCertificatePolicyDTO.builder()
                                         ...
                                         .withRevocationCheckEnabled(clientCertPolicy.revocationCheckEnabled())
                                         .withCrlDistributionPointUsageEnabled(clientCertPolicy.useCertCRL())
                                         .withCrlDistributionPointOverride(crlUrl)
                                         .build();
    }

    public static ClientCertPolicy getClientCertificatePolicy(ClientCertificatePolicyDTO certPolicyDTO) {
        ...
        if (certPolicyDTO.getCertPolicyOIDs() != null) {
            ...
            String[] objectIdentifiers = certPolicyDTO.getCertPolicyOIDs().toArray(new String[totalObjectIds]);
            certPolicy.setOIDs(objectIdentifiers);
        }

        if (certPolicyDTO.getTrustedCACertificates() != null) {
            // trusted certificates
            int totalTrustedCertificates = certPolicyDTO.getTrustedCACertificates().size();
            List<Certificate> certs = CertificateMapper.getCertificates(certPolicyDTO.getTrustedCACertificates());
        ...
        try {
            ...
            certPolicy.setUseOCSP(certPolicyDTO.isOcspEnabled() != null ? certPolicyDTO.isOcspEnabled() : false);
            certPolicy.setUseCRLAsFailOver(certPolicyDTO.isFailOverToCrlEnabled() != null ? certPolicyDTO.isFailOverToCrlEnabled() : false);
            if (certPolicyDTO.getOcspUrlOverride() != null) {
                certPolicy.setOCSPUrl(new URL(certPolicyDTO.getOcspUrlOverride()));
            }

            // crl related settings
            certPolicy.setRevocationCheckEnabled(certPolicyDTO.isRevocationCheckEnabled() != null ? certPolicyDTO.isRevocationCheckEnabled() : true);
            certPolicy.setUseCertCRL(certPolicyDTO.isCrlDistributionPointUsageEnabled() != null ? certPolicyDTO.isCrlDistributionPointUsageEnabled() : true);
            if (certPolicyDTO.getCrlDistributionPointOverride() != null) {
                certPolicy.setCRLUrl(new URL(certPolicyDTO.getCrlDistributionPointOverride()));
            }
        } catch (MalformedURLException e) {
            throw new DTOMapperException("Failed to convert ClientCertPolicyDTO to ClientCertPolicy", e);
        }

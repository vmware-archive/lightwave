    ...
    )
{
    ...
    if (pszOurs == NULL || LwCAGetStringLengthW(pszOurs, &len1) != LWCA_SUCCESS)
    {
        return FALSE;
    }

    if (pszTheirs == NULL || LwCAGetStringLengthW(pszTheirs, &len2) != LWCA_SUCCESS ||
        len2 != len1)
    {
        ret = 1;
        p = pszOurs;
    ...
    )
{
    ...
    if (pszStr)
    {
        len = LwCAStringLenA(pszStr);
        if (len > 0)
        {
            for (start = 0; start < len && LWCA_ASCII_SPACE(pszStr[start]); start++);
            for (end = len - 1; end > 0 && LWCA_ASCII_SPACE(pszStr[end]); end--);
...
DWORD
LwCAGetUTCTimeString(PSTR *pszTimeString)
{
...
cleanup:
    return dwError;
error:
    LWCA_SAFE_FREE_STRINGA(szDateTime);
    goto cleanup;
}

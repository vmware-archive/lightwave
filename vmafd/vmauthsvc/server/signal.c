    ...
    )
{
    ...
    action.sa_handler = VmAuthsvcInterruptHandler;
    ...
    while (1)
    {
        ...
        switch (which_signal)
        {
            ...
            case SIGTERM:
            {
                VmAuthsvcLog( VMAUTHSVC_DEBUG_ANY, "Handled SIG[%d]\n",which_signal);
                goto error;
            }
            case SIGPIPE:
            {
                VmAuthsvcLog( VMAUTHSVC_DEBUG_ANY, "Handled SIGPIPE");

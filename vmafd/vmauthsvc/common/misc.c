    ...
    PSTR pszString)
{
    ...
    iLen = VmAuthsvcStringLenA(pszString);

    BAIL_ON_VMAUTHSVC_INVALID_POINTER(pszString, dwError);

    for (i=0; i<iLen; i++)
    {
        VMAUTHSVC_ASCII_LOWER_TO_UPPER(pszString[i]);
    }
    ...
    PSTR pszString)
{
    ...
    for (i=0; i<iLen; i++)
    {
        VMAUTHSVC_ASCII_UPPER_TO_LOWER(pszString[i]);
    }
...
error:
    ...
    *ppszCanonicalHostname = NULL;
    ...
    VMAUTHSVC_SAFE_FREE_MEMORY(pszCanonicalHostname);

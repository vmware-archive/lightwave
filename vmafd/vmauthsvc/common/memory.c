...
error:
    VMAUTHSVC_SAFE_FREE_MEMORY(pMemory);
    pMemory = NULL;
    ...
    )
{
    ...
    {
        dwError = VmAuthsvcAllocateMemory(dwSize, &pNewMemory);
        BAIL_ON_VMAUTHSVC_ERROR(dwError);
    }
    ...
    )
{
    ...
    dwError = VmAuthsvcReallocateMemory(pMemory, ppNewMemory, dwNewSize);
    ...
    )
{
    ...
    dwLen = VmAuthsvcStringLenA(pszString);
    // + 1 for \'0'
    dwError = VmAuthsvcAllocateMemory(dwLen + 1, (PVOID*)&pszNewString);
    BAIL_ON_VMAUTHSVC_ERROR(dwError);
...
error:
    ...
    VMAUTHSVC_SAFE_FREE_MEMORY(pszNewString);
...
{
    VMAUTHSVC_SAFE_FREE_MEMORY(pszString);
    ...
    )
{
    ...
    {
        VmAuthsvcFreeStringA(ppszString[dwCnt]);
    }
    ...
    )
{
    ...
    if (iInitSize - iFinalSize <= 0)
    {
        ...
        dwError = VmAuthsvcReallocateMemory(
                pszBuf,
                (PVOID*)&pszBuf,
                iFinalSize + 1);
        BAIL_ON_VMAUTHSVC_ERROR(dwError);
...
error:
    ...
    VMAUTHSVC_SAFE_FREE_MEMORY(pszBuf);
    ...
    */
    ...
    dwError =
        VmAuthsvcAllocateMemory( (iSize*sizeof(CHAR)), (PVOID *)&pszAnsiString );
    BAIL_ON_VMAUTHSVC_ERROR(dwError);
...
error:
    ...
    VMAUTHSVC_SAFE_FREE_MEMORY(pszAnsiString);
...
ULONG
VmAuthsvcAllocateSidFromCString(
    PCSTR pszSidString,
    PSID* ppSid
    )
{
    ...
    */
    ...
    // sid, with regular VMAuthsvcFreeMemory, but on windows
    ...
    dwError = VmAuthsvcAllocateMemory( sidLength, ((PVOID*)(&cloneSid)));
    BAIL_ON_VMAUTHSVC_ERROR(dwError);
...
error:
    ...
    VMAUTHSVC_SAFE_FREE_MEMORY(cloneSid);
    if( originalSid != NULL )
    {
...
ULONG
VmAuthsvcAllocateCStringFromSid(
    PSTR* ppszStringSid,
    PSID pSid
    )
{
    ...
    */
    ...
    // callers of this function are prepared to free the
    // string sid, with regular VMAuthsvcFreeString, but on windows
    // original Sid needs to be freed with LocalFree...
    // therefore we will clone the string being returned ...
    dwError = VmAuthsvcAllocateStringA(origSid, &cloneSid);
    BAIL_ON_VMAUTHSVC_ERROR(dwError);
...
error :
    ...
    VMAUTHSVC_SAFE_FREE_STRINGA(cloneSid);
    if( origSid != NULL )
    {
        LocalFree(origSid);
        origSid = NULL;
